...
func (pq *ProviderQuery) QuerySubscribes() *SubscribeQuery {
	query := (&SubscribeClient{config: pq.config}).Query()
...
func (pq *ProviderQuery) Clone() *ProviderQuery {
	...
	return &ProviderQuery{
		...
		withSubscribes: pq.withSubscribes.Clone(),
...
func (pq *ProviderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Provider, error) {
	...
	if query := pq.withSubscribes; query != nil {
		if err := pq.loadSubscribes(ctx, query, nodes,
			func(n *Provider) { n.Edges.Subscribes = []*Subscribe{} },
			func(n *Provider, e *Subscribe) { n.Edges.Subscribes = append(n.Edges.Subscribes, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProviderQuery) loadSubscribes(ctx context.Context, query *SubscribeQuery, nodes []*Provider, init func(*Provider), assign func(*Provider, *Subscribe)) error {
	...
	query.Where(predicate.Subscribe(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(provider.SubscribesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.provider_subscribes
		if fk == nil {
			return fmt.Errorf(`foreign-key "provider_subscribes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "provider_subscribes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *ProviderQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Provider, init func(*Provider), assign func(*Provider, *User)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Provider)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(provider.UserColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.provider_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "provider_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "provider_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

...
func (sq *SubscribeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscribe, error) {
	...
	if query := sq.withProvider; query != nil {
		if err := sq.loadProvider(ctx, query, nodes, nil,
			func(n *Subscribe, e *Provider) { n.Edges.Provider = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := sq.withNodes; query != nil {
		if err := sq.loadNodes(ctx, query, nodes,
			func(n *Subscribe) { n.Edges.Nodes = []*Node{} },
			func(n *Subscribe, e *Node) { n.Edges.Nodes = append(n.Edges.Nodes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SubscribeQuery) loadProvider(ctx context.Context, query *ProviderQuery, nodes []*Subscribe, init func(*Subscribe), assign func(*Subscribe, *Provider)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Subscribe)
	for i := range nodes {
		if nodes[i].provider_subscribes == nil {
			continue
		}
		fk := *nodes[i].provider_subscribes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "provider_subscribes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *SubscribeQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Subscribe, init func(*Subscribe), assign func(*Subscribe, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subscribe.UserColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.subscribe_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "subscribe_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "subscribe_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (sq *SubscribeQuery) loadNodes(ctx context.Context, query *NodeQuery, nodes []*Subscribe, init func(*Subscribe), assign func(*Subscribe, *Node)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Subscribe)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Node(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subscribe.NodesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.subscribe_nodes
		if fk == nil {
			return fmt.Errorf(`foreign-key "subscribe_nodes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "subscribe_nodes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

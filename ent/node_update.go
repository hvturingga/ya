...
func (nu *NodeUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := nu.mutation.Group(); ok {
		_spec.SetField(node.FieldGroup, field.TypeString, value)
	}
	if value, ok := nu.mutation.Name(); ok {
		_spec.SetField(node.FieldName, field.TypeString, value)
	}
	if nu.mutation.SubscribeCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   node.SubscribeTable,
			Columns: []string{node.SubscribeColumn},
			Bidi:    false,
	...
	if nodes := nu.mutation.SubscribeIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   node.SubscribeTable,
...
func (nuo *NodeUpdateOne) SetSubscribeID(id int) *NodeUpdateOne {
	nuo.mutation.SetSubscribeID(id)
	return nuo
}
...
func (nuo *NodeUpdateOne) SetSubscribe(s *Subscribe) *NodeUpdateOne {
	return nuo.SetSubscribeID(s.ID)
}
...
func (nuo *NodeUpdateOne) sqlSave(ctx context.Context) (_node *Node, err error) {
	...
	if value, ok := nuo.mutation.Group(); ok {
		_spec.SetField(node.FieldGroup, field.TypeString, value)
	}
	if value, ok := nuo.mutation.Name(); ok {
		_spec.SetField(node.FieldName, field.TypeString, value)
	}
	if nuo.mutation.SubscribeCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   node.SubscribeTable,
	...
	if nodes := nuo.mutation.SubscribeIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   node.SubscribeTable,

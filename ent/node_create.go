...
func (nc *NodeCreate) check() error {
	if _, ok := nc.mutation.Group(); !ok {
		return &ValidationError{Name: "group", err: errors.New(`ent: missing required field "Node.group"`)}
	}
...
func (nc *NodeCreate) createSpec() (*Node, *sqlgraph.CreateSpec) {
	...
	if value, ok := nc.mutation.Group(); ok {
	...
	if nodes := nc.mutation.SubscribeIDs(); len(nodes) > 0 {

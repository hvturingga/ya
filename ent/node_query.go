...
func (nq *NodeQuery) QuerySubscribe() *SubscribeQuery {
	query := (&SubscribeClient{config: nq.config}).Query()
...
func (nq *NodeQuery) Clone() *NodeQuery {
	...
	return &NodeQuery{
		...
		withSubscribe: nq.withSubscribe.Clone(),
...
func (nq *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	var (
		...
		_spec       = nq.querySpec()
		loadedTypes = [1]bool{
			nq.withSubscribe != nil,
		}
	)
	if nq.withSubscribe != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := nq.withSubscribe; query != nil {
		if err := nq.loadSubscribe(ctx, query, nodes, nil,
			func(n *Node, e *Subscribe) { n.Edges.Subscribe = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NodeQuery) loadSubscribe(ctx context.Context, query *SubscribeQuery, nodes []*Node, init func(*Node), assign func(*Node, *Subscribe)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Node)
	for i := range nodes {
		if nodes[i].subscribe_nodes == nil {
			continue
		}
		fk := *nodes[i].subscribe_nodes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "subscribe_nodes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
